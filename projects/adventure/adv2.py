...
# Update direction moving from room1 to room2
def updatedirdict(visited, room1, room2, direction):
    reverse_direction = {'n': 's',
                         's': 'n',
                         'e': 'w',
                         'w': 'e'}

    # Do book keeping for rooms.
    if room1.id not in visited:
        dir_dict = getdirdict(room1.getExits())
        visited[room1.id] = dir_dict

    if room2.id not in visited:
        dir_dict = getdirdict(room2.getExits())
        visited[room2.id] = dir_dict

    visited[room1.id][direction] = room2.id
    visited[room2.id][reverse_direction[direction]] = room1.id


# Perform depth first traversal and update visited path information
# Returns the last node where no further traversal is possible.
def godeep(player, visited, traversalPath):
    # Create empty set
    visited_rooms = set()
    # Create empty Stack
    stack = Stack()
    ...
    if player.currentRoom.id not in visited:
        dir_dict = getdirdict(player.currentRoom.getExits())
        visited[player.currentRoom.id] = dir_dict
        ...
        if room.id not in visited_rooms:
            ...
            while len(room_directions) > 0:
                ...
                if value == '?':
                    ...
                    # Player needs to move forward and back to get
                    # next room information.
                    player.travel(direction)
                    next_room = player.currentRoom
                    player.travel(reverse_direction[direction])

                    # Update room direction mapping
                    updatedirdict(visited, room, next_room, direction)
...
# Perform Breadth First Search for finding the closest room with
# unexplored direction.
def searchnext(player, visited, traversalPath):
    # Create empty set
    visited_room_ids = set()

    # Create empty Queue and add initial path
    queue = Queue()
