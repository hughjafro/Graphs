...
from dungeon_util import Stack, Queue
...
# def room_dir(directions):
#     num_exits = len(exits)
#     i = 0
#     # While there are still exits
#     while i < num_exits:
#         if graph[room][exits[i]] == '?':
#             return exits[i]
...
# Use the opposite direction to move from room to room
...
# def update_dir(visited, room1, room2, direction):
...
#         room_dir = find_dir(room1.getExits())
#         visited[room1.id] = find_dir

#     if room2.id not in visited:
#         room_dir = find_dir(room2.getExits())
#         visited[room2.id] = find_dir

#     visited[room1.id][direction] = room2.id
#     visited[room2.id][opposite_direction[direction]] = room1.id

def update_direction(visited, room1, room2, direction):
    opposite_dir = {'n': 's',
                    's': 'n',
                    'e': 'w',
                    'w': 'e'}
    ...
    if room1.id not in visited:
        room_dir = find_dir(room1.getExits())
        visited[room1.id] = room_dir

    if room2.id not in visited:
        room_dir = find_dir(room2.getExits())
        visited[room2.id] = room_dir

    visited[room1.id][direction] = room2.id
    visited[room2.id][opposite_dir[direction]] = room1.id

# Use DFT to move thru maze until it dead ends
def dft_graph(player, visited, traversalPath):
    # Create an empty stack and push the starting vertex ID
    s = Stack()
    s.push(player.currentRoom)  # starting_vertex, add room

    # Create an empty Set to store visited vertices
    visited_rooms = set()

    if player.currentRoom.id not in visited:
        room_dir = find_dir(player.currentRoom.getExits())
        visited[player.currentRoom.id] = room_dir
    
    # While the stack is not empty...
    while s.size() > 0:
        # Pop the first vertex
        room = s.pop()  # from class in util.py
        # If the vertex has not been visited...
        if room.id not in visited_rooms:
            # Mark it as visited
            print(room.id)
            visited_rooms.add(room.id)
            # Then find &add all of its neighbors to top of the stack
            # for next_vert in self.get_neighbors(v):
            #     s.push(next_vert)
            room_directions = visited[room.id]
            # return list with all dict keys/values
            ...
            while len(room_directions) > 0:
                ...
                # If ? then move in this direction
                if value == '?':
                    opposite_dir = {'n': 's',
                                    's': 'n',
                                    'e': 'w',
                    ...
                    next_vert = player.currentRoom
                    player.travel(opposite_dir[direction])

                    # Append to traversalPath and update currentRoom
                    update_direction(visited, room, next_vert, direction)
...
def bfs(player, visited, traversalPath):
    ...
    #             return path
    #         # Mark it as visited
    #         visited_room_id.add(v)
    #         # Then add A PTH TO its neighbors to the back of queue
    #         for direction, next_vertid in room_directions(v):
    #             # COPY THE PATH
    #             path_copy = path.copy()
    #             # APPEND THE NEIGHBOR TO THE BACK
    #             path_copy.append(next_vertid)
    #             q.enqueue(path_copy)
...
for i in range(1):
    ...
    while True:
        ...
        if not bfs(player, visited, traversalPath):
            break
